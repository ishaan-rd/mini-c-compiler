...
assignment_exp:  identifier OP_ASS arithmetic_exp			{id_present($1); check_type($1, I); add_int($1, $3);}
		...
		| identifier OP_ASS function_call					{id_present($1); id_present($3); check_type($1, $3);}
		| identifier OP_ASS OP_ADR identifier				{id_present($1); id_present($4); int x =  type_get($4); check_type($1, x * x);  add_ptr($1, $4;}
...
declare: identifier								{ insert(table, $1, current_dt, scope); }
		...
		| identifier OP_ASS OP_ADR identifier	{ insert(table, $1, current_dt, scope); int x = type_get($4); check_both_type(current_dt, x*x); point_val($1, $4);}
		;
